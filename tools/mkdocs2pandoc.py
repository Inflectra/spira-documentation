...
def main():
    ...
    if sys.version_info.major == 2:
        ...
        out = open(sys.stdout.fileno(), mode='w',
                   encoding=args.encoding, buffering=1)
    ...
    print("OK, now go parse an yml")

    if args.outfile:
        try:
            out = codecs.open(args.outfile, 'w', encoding=args.encoding)
        except IOError as e:
            print("Couldn't open %s for writing: %s" %
                  (args.outfile, e.strerror), file=sys.stderr)
