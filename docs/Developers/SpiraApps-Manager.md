...
=== "Explanation"
    ...
    **Details page events**:
    ...
    - **registerEvent_dataPreSave**: Registers an event handler on a details page to trigger after a user has started a save operation, but before the save is submitted to the database. This allows you to validate or modify the values which will be saved, such as to perform a formula or enforce field level permission workflows on this page.
