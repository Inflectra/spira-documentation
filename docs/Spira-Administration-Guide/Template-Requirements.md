...
## Statuses
Requirement statuses are fixed. Admins cannot add or rename requirement statuses. The requirement status page lets template admins manage how these statuses are displayed to users on relevant planning boards.
...
- Under Review (this is shown first because it does not have a position set)
- Planned (because it has a position of 1)
- Requested (because it has a position of 2)
