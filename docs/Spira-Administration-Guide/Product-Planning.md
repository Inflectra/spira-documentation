...
* **Test Case Execution**: the following settings affect the test execution rules / experience of all testers in the products
    ...
    - **Every test step that fails must have an Incident**: (default = no) turn this setting on to require an incident only when a test step is marked as Failed. Unlike the previous setting, this does not apply to Blocked, Caution, or N/A statuses - only explicit failures require an incident. When this setting is on and a tester marks a step as Failed, they must either link an existing incident or create a new one before proceeding. This is useful for organizations that want to track true failures while allowing testers to mark steps as Blocked, Caution, or N/A without mandatory incident creation.
    ...
    - **Disable actual duration recording (always report 0 minutes)**: (default = no) when turned on, the system records 0.0h as the actual duration for every test case executed, regardless of how long the tester spent. This is intended for organizations operating in jurisdictions where local laws restrict or prohibit tracking granular, task-by-task time spent by individual workers. Turning this on ensures no per-test-case timing data is captured.
