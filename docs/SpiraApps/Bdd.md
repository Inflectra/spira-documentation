...
If you can create documents, you can use the "BDD Actions" > "Export Feature" toolbar button to export the current artifact's steps or mitigations as a `.feature` file. You will be asked to name the feature file, and it will then be saved to Spira's document repository for that product. The first line of the feature file will be the name of the artifact itself (e.g. the requirement name). 
