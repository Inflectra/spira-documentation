...
??? success "New Features"
    - Fix planning board Incident Cards in the same column as Requirement cards not always staying where dragged and dropped [RQ:5403]

??? bug "Bug fixes and enhancements"
    - Add ability for SpiraAppManager to automatically save the current artifact directly using a new SpiraAppManager.saveForm() function [IN:11077]
    ...
    - Fix being unable to see a test case's embedded images when exported to another product, if the user is not a member of the source product [IN:11377]
    - Fix being unable to see an incident's embedded images when exported to another product, if the user is not a member of the source product [IN:11375]
    - Fix dragging cards in the Unassigned Items column on product boards not placing the cards in the correct place, but instead always putting them at the bottom [IN:11416]
    ...
    - Fix Test Set Export not being able to export a test set that contains test cases from other products without error, by not exporting such test cases [IN:11478]
    - Fix Test Set Export so that so exported Test Set Test Cases with Parameters have their values properly set for test execution [IN:11458]
    - Fix Test Set Export so that Test Case Automation files are kept in place [IN:11459]
    - Fix Test Set Export when Test Set has same Test Case in the Test Set multiple times [IN:11453]
    ...
    - Impose a strict limit on the maximum number of cards that can be retrieved for any column in total (including across rows) on all boards to 500, to improve performance [IN:11410]
