...
- Ensure the following settings are applied:
    ...
    - **Send Email Importer Separator?**: (off by default) Set this to on to makes it possible to reply to emails by Spira adding a separator line at the bottom of each email. This ensure replies to the email are properly handled by Spira.
...
!!! attention "Important Notes"
    - If upgrading from v1 or v2 to v3 (the latest) you will need to edit the configuration for any Spira applications and change the password to the account's API key. Until this is done, email will not be imported.
