...
This guide explains how to use Inflectra's "Jira Migration Tool" for importing projects, versions, requirements, issues, tasks and associated attachments from Atlassian Jira to Spira. This tool works with **Jira Cloud**, **Jira Server** and **Jira Data Center** editions.
...
This guide outlines how to install the migration tool for Jira onto a workstation so that you can then migrate whole projects from Jira to Spira. It assumes that you already have a working installation of Spira v6.0 or later and a working version of Jira Cloud, Server or Data Center. 
...
Now that you have installed the migration tool, you can launch it at any time by going to Start \> Programs \> Inflectra \> Spira \> Tools \> Jira Importer. This will launch the migration tool application itself:
...
### Mapping Jira Issues Types to Spira Artifacts

![](img/jira-importer-7.png)

On this page you will map the different Spira artifacts to the different issue types in Jira. Currently, the following artifact types in Spira can be mapped to Jira issues:
...
Once you have mapped the artifacts, click the **Start Import** button to actually begin the process of importing the various artifacts from Jira into Spira.

### Importer Progress

![](img/jira-importer-8.png)

Note that the importer will automatically create a new project in Spira to hold all the artifacts with the same name as that used in Jira.
...
### Requirements
Any of the Jira issue types that are mapped to requirements in Spira:
...
### Tasks
Any of the Jira issue types that are mapped to tasks in Spira:

![](img/jira-tasks-1.png)

Will be imported into Spira as types of task:

![](img/jira-tasks-2.png)

### Incidents
Any of the Jira issue types that are mapped to incidents in Spira:
...
Note that it will convert XRAY parameters from the formal used in XRAY to those in Spira. It also migrates over the test parameter values from the corresponding XRAY data set.
...
### Test Sets and Test Plans
The migration tool will migrate the XRAY test sets and test plans into Spira as test sets:

![](img/jira-xray-importer-4.png)

The test sets will be imported into a flat list in Spira (that you can organize by folder, post-migration), together with the mapping to the various test cases included in the XRAY test set or test plan.

### Test Executions
The XRAY test executions and sub-test executions will be imported into Spira together with their mapped test cases and test runs. These will be migrated into Spira as a test run. The various fields and statuses are faithfully migrated.
